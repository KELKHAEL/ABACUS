...
export default function TeacherHome({ navigation }) {
  ...
  const displayName = isAdmin ? "Admin" : (user?.fullName || "Instructor");
  ...
  const fetchAnnouncements = async () => {
    if (!user) return;
    try {
      let url = isAdmin ? `${API_URL}/announcements/all` : `${API_URL}/announcements/admin-to-instructor/${user.id}`;
  ...
  const onRefresh = useCallback(() => {
    ...
    if (activeTab === 'Announcements') {
      ...
      fetchGradebookData().finally(() => setRefreshing(false));
    } else {
      setRefreshing(false);
    }
  }, [activeTab]);
    ...
    <ScrollView contentContainerStyle={styles.scrollContent}>
      <View style={styles.header}>
        <View style={styles.headerTextContainer}>
          <Text style={styles.greetingText} numberOfLines={1} adjustsFontSizeToFit>Hello, {displayName}</Text>
          <Text style={styles.subText}>{isAdmin ? "System Administrator" : "Class Overview"}</Text>
        </View>
      ...
      <View style={styles.webNotice}>
        <Ionicons name="desktop-outline" size={20} color="#155724" />
        <Text style={styles.webText}>For file uploads and quiz creation, please use the <Text style={{fontWeight:'bold'}}>ABACUS Web Portal</Text>.</Text>
      </View>

      <Text style={styles.sectionTitle}>Course Content Preview</Text>
      <View style={styles.listContainer}>
        {allModules.map((item) => (
          <ModuleCard key={item.id} title={item.title} category={item.category} color={item.color} onPress={() => navigation.navigate('ModuleDetail', { moduleTitle: item.title, moduleColor: item.color, topics: item.topics || [] })} />
        ))}
      </View>
  ...
  const renderGradebook = () => (
    <ScrollView contentContainerStyle={styles.scrollContent} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={onRefresh} />}>
      ...
      <Text style={styles.subText}>{isAdmin ? "Global School Roster" : "Your Assigned Classes"}</Text>

      {/* DYNAMIC FILTERS SECTION */}
      <View style={styles.gradebookFilterContainer}>
        {isAdmin ? (
          <>
            {/* Dynamic Programs */}
            <ScrollView horizontal showsHorizontalScrollIndicator={false} style={{marginBottom: 10}}>
                <TouchableOpacity style={[styles.gbChip, adminProg === 'ALL' && styles.chipActive]} onPress={() => setAdminProg('ALL')}>
                    <Text style={[styles.chipText, adminProg === 'ALL' && styles.chipTextActive]}>ALL</Text>
                </TouchableOpacity>
                {academicPrograms.map(p => (
                    <TouchableOpacity key={p.id} style={[styles.gbChip, adminProg === p.name && styles.chipActive]} onPress={() => setAdminProg(p.name)}>
                        <Text style={[styles.chipText, adminProg === p.name && styles.chipTextActive]}>{p.name.replace('Bachelor of Science in', 'BS')}</Text>
                    </TouchableOpacity>
                ))}
            </ScrollView>
      ...
      <View style={{marginTop: 15, gap: 12}}>
        {loadingGrades ? (
          <ActivityIndicator size="large" color="#104a28" style={{marginTop: 30}} />
        ) : filteredStudents.length === 0 ? (
          <View style={styles.emptyState}>
             <Ionicons name="people-outline" size={50} color="#ccc" />
             <Text style={styles.emptyText}>No students found in this selection.</Text>
          </View>
        ) : (
          filteredStudents.map((s, idx) => (
             <TouchableOpacity key={idx} style={styles.studentCard} onPress={() => navigation.navigate('StudentGradesScreen', { studentId: s.id, studentName: s.full_name, studentNo: s.student_id })}>
        ...
        <ScrollView contentContainerStyle={styles.scrollContent} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={onRefresh} />}>
          <View style={{flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center'}}>
            <View>
              ...
              <Text style={styles.subText}>{isAdmin ? "Global Campus Feed" : "Registrar & Campus Updates"}</Text>
            </View>
            <View style={{flexDirection: 'row', gap: 10}}>
              <TouchableOpacity onPress={onRefresh} style={styles.filterBtn}><Ionicons name="refresh" size={20} color="#104a28" /></TouchableOpacity>
              <TouchableOpacity onPress={() => setShowFilterModal(true)} style={[styles.filterBtn, isFiltered && styles.filterBtnActive]}><Ionicons name="filter" size={20} color={isFiltered ? "white" : "#104a28"} /></TouchableOpacity>
            </View>
          </View>

          <View style={{marginTop: 20, gap: 15}}>
            {loadingAnnounce ? <ActivityIndicator size="large" color="#104a28" style={{marginTop: 20}} />
            : filteredAnnouncements.length === 0 ? <View style={styles.emptyState}><Ionicons name="notifications-off-outline" size={50} color="#ccc" /><Text style={styles.emptyText}>No announcements found.</Text></View>
            : filteredAnnouncements.map((item) => <AnnouncementCard key={item.id} item={item} />)}
          </View>
          <View style={styles.spacer} />
        </ScrollView>
            ...
            <View style={styles.modalOverlay}>
                <View style={styles.modalContent}>
                    ...
                    <View style={styles.modalFooter}>
                        <TouchableOpacity style={styles.clearBtn} onPress={clearFilters}><Text style={styles.clearBtnText}>Reset All</Text></TouchableOpacity>
                        <TouchableOpacity style={styles.applyBtn} onPress={() => setShowFilterModal(false)}><Text style={styles.applyBtnText}>Apply Filters</Text></TouchableOpacity>
                    </View>
                </View>
            </View>
        </Modal>
      ...
      <View style={styles.bottomNavContainer}>
        <View style={styles.bottomNav}>
          <TouchableOpacity style={[styles.navItem, activeTab === 'Home' && styles.navItemActive]} onPress={() => setActiveTab('Home')}>
            ...
            {activeTab === 'Home' && <Text style={styles.navTextActive}>Content</Text>}
          </TouchableOpacity>

          <TouchableOpacity style={[styles.navItem, activeTab === 'Simulations' && styles.navItemActive]} onPress={() => setActiveTab('Simulations')}>
          ...
          <TouchableOpacity style={[styles.navItem, activeTab === 'Gradebook' && styles.navItemActive]} onPress={() => setActiveTab('Gradebook')}>
            <Ionicons name={activeTab === 'Gradebook' ? "book" : "book-outline"} size={22} color={activeTab === 'Gradebook' ? "white" : "#666"} />
            {activeTab === 'Gradebook' && <Text style={styles.navTextActive}>Roster</Text>}
          </TouchableOpacity>

          <TouchableOpacity style={[styles.navItem, activeTab === 'Announcements' && styles.navItemActive]} onPress={() => setActiveTab('Announcements')}>
...
const styles = StyleSheet.create({
  ...
  webNotice: { marginBottom: 25, padding: 15, backgroundColor: '#e6f4ea', borderRadius: 12, flexDirection: 'row', alignItems: 'center', gap: 10, borderColor: '#c3e6cb', borderWidth: 1 },
  webText: { flex: 1, fontSize: 12, color: '#155724', lineHeight: 18 },
  ...
  gradebookFilterContainer: { marginTop: 15, marginBottom: 5 },
  gbChip: { paddingVertical: 8, paddingHorizontal: 16, borderRadius: 20, backgroundColor: '#f3f4f6', borderWidth: 1, borderColor: '#e5e7eb', marginRight: 10 },
  studentCard: { backgroundColor: '#fff', padding: 15, borderRadius: 12, flexDirection: 'row', alignItems: 'center', shadowColor: "#000", shadowOffset: { width: 0, height: 1 }, shadowOpacity: 0.05, shadowRadius: 3, elevation: 1 },
  studentAvatar: { width: 40, height: 40, borderRadius: 20, backgroundColor: '#e6f4ea', justifyContent: 'center', alignItems: 'center', marginRight: 12 },
  studentName: { fontSize: 15, fontWeight: 'bold', color: '#333' },
  studentInfo: { fontSize: 12, color: '#666', marginTop: 2 },
  ...
  bottomNav: { flexDirection: 'row', backgroundColor: '#fff', width: '94%', paddingVertical: 12, paddingHorizontal: 15, borderRadius: 35, justifyContent: 'space-around', alignItems: 'center', shadowColor: "#000", shadowOffset: { width: 0, height: 4 }, shadowOpacity: 0.1, shadowRadius: 10, elevation: 5 },
  ...
  navTextActive: { color: '#fff', fontWeight: '600', fontSize: 12 },
