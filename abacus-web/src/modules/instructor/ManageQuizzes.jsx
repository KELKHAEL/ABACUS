...
import { 
  ...
  FileText, Edit, BookOpen, Clock, Settings
} from 'lucide-react';
...
export default function ManageQuizzes() {
  ...
  const [editFormData, setEditFormData] = useState({
    id: null, title: '', description: '', targetYear: 'ALL', targetSection: 'ALL', difficulty: 'Easy'
  });
  ...
  const fetchQuizzes = async () => {
    ...
    try {
      ...
      // Reusing your dashboard endpoint to get the instructor's quizzes
  ...
  const changeQuizStatus = async (id, newStatus, e) => {
    ...
    if (window.confirm(`${actionName} this quiz?`)) {
      ...
      } catch (error) { alert("Network failed. Is the server running?"); }
    }
  };
    ...
    <div className="instructor-dashboard-container">
      ...
      <div className="inst-dashboard-header">
        <div className="header-left">
            ...
            <p>Manage, edit, or permanently delete your assessments.</p>
        </div>
          ...
          {displayedQuizzes.map((quiz) => (
            <div key={quiz.id} className="inst-quiz-card">
              ...
              <div className="card-actions">
                {viewMode === 'active' ? (
                  <>
                    <button className="btn-action btn-view" onClick={() => setViewingQuiz(quiz)} title="Preview Details"><Eye size={16}/></button>
          ...
          <div className="modal-content" onClick={e => e.stopPropagation()} style={{maxWidth: '550px', padding: 0, overflow: 'hidden'}}>
            <div style={{backgroundColor: '#f8f9fa', padding: '24px', borderBottom: '1px solid #e5e7eb', display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start'}}>
                <div>
                    ...
                    <div style={{display: 'flex', gap: '12px', marginTop: '12px'}}>
                        {viewingQuiz.target_year && <span style={{fontSize: '12px', background: '#ecfccb', color: '#365314', padding: '4px 8px', borderRadius: '4px', fontWeight: 'bold'}}>Target: Year {viewingQuiz.target_year} - {viewingQuiz.target_section}</span>}
                        {viewingQuiz.difficulty && <span style={{fontSize: '12px', background: '#e0f2fe', color: '#075985', padding: '4px 8px', borderRadius: '4px', fontWeight: 'bold'}}>{viewingQuiz.difficulty}</span>}
                    </div>
                </div>
            ...
            <div style={{padding: '24px'}}>
                <div style={{marginBottom: '20px'}}>
                    <h4 style={{fontSize: '14px', color: '#6b7280', marginBottom: '8px', display: 'flex', alignItems: 'center', gap: '6px', textTransform: 'uppercase', letterSpacing: '0.5px'}}><FileText size={16}/> Description</h4>
                    <p style={{fontSize: '15px', color: '#374151', lineHeight: '1.6', background: '#f9fafb', padding: '16px', borderRadius: '8px', border: '1px solid #f3f4f6'}}>{viewingQuiz.description || "No description provided for this assessment."}</p>
                </div>
                ...
                <form onSubmit={handleSaveEdit}>
                    <div style={{padding: '24px', display: 'flex', flexDirection: 'column', gap: '16px'}}>
                        <div>
                            ...
                            <input required value={editFormData.title} onChange={e => setEditFormData({...editFormData, title: e.target.value})} style={{width: '100%', padding: '12px', border: '1px solid #d1d5db', borderRadius: '8px', fontSize: '15px', outline: 'none', boxSizing: 'border-box'}}/>
                        </div>
                        <div>
                            <label style={{display: 'block', fontSize: '13px', fontWeight: 'bold', color: '#4b5563', marginBottom: '6px'}}>Description</label>
                            <textarea value={editFormData.description} onChange={e => setEditFormData({...editFormData, description: e.target.value})} rows="3" style={{width: '100%', padding: '12px', border: '1px solid #d1d5db', borderRadius: '8px', fontSize: '14px', outline: 'none', resize: 'vertical', boxSizing: 'border-box', fontFamily: 'inherit'}}/>
                        </div>
                        <div style={{display: 'flex', gap: '16px'}}>
                            <div style={{flex: 1}}>
                                <label style={{display: 'block', fontSize: '13px', fontWeight: 'bold', color: '#4b5563', marginBottom: '6px'}}>Target Year</label>
                                <select value={editFormData.targetYear} onChange={e => setEditFormData({...editFormData, targetYear: e.target.value})} style={{width: '100%', padding: '12px', border: '1px solid #d1d5db', borderRadius: '8px', fontSize: '14px', outline: 'none', background: 'white', cursor: 'pointer'}}>
                            ...
                            <div style={{flex: 1}}>
                                ...
                                <select value={editFormData.targetSection} onChange={e => setEditFormData({...editFormData, targetSection: e.target.value})} style={{width: '100%', padding: '12px', border: '1px solid #d1d5db', borderRadius: '8px', fontSize: '14px', outline: 'none', background: 'white', cursor: 'pointer'}}>
                        ...
                        <div>
                            ...
                            <select value={editFormData.difficulty} onChange={e => setEditFormData({...editFormData, difficulty: e.target.value})} style={{width: '100%', padding: '12px', border: '1px solid #d1d5db', borderRadius: '8px', fontSize: '14px', outline: 'none', background: 'white', cursor: 'pointer'}}>
                    ...
                    <div style={{padding: '16px 24px', background: '#f9fafb', borderTop: '1px solid #e5e7eb', display: 'flex', gap: '12px', justifyContent: 'flex-end'}}>
                        <button type="button" onClick={() => setShowEditModal(false)} style={{padding: '10px 20px', borderRadius: '6px', border: '1px solid #d1d5db', background: 'white', color: '#374151', fontWeight: '600', cursor: 'pointer'}}>Cancel</button>
                        <button type="submit" style={{padding: '10px 20px', borderRadius: '6px', border: 'none', background: '#eab308', color: '#422006', fontWeight: 'bold', cursor: 'pointer'}}>Save Changes</button>
                    </div>
                </form>
            </div>
        </div>
